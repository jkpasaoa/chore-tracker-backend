...
const createChore = async (choreToAdd) => {
  try {
    ...
    const newChore = await db.one('INSERT INTO chores (choreToAdd.name, choreToAdd.description, choreToAdd.due_date, choreToAdd.status, choreToAdd.points, choreToAdd.priority, choreToAdd.category) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *', [name, description, due_date, status, points, priority, category]);
    return newChore;
  } catch (error) {
    return error;
  }
